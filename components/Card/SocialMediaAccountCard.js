...
const SocialMediaAccountCard = ({name, username}) => {
  return (
    <Link
      href={'/'}
      target={'_blank'}
    ...
    >
      <Flex alignItems={'center'} gap={4}>
        ...
        <Flex flexDir={'column'}>
          <Text fontWeight={'bold'} fontSize={'xl'} lineHeight={'shorter'}>{name}</Text>
          <Text fontFamily={'Open Sans'} lineHeight={'shorter'} fontSize={'sm'} color={'gray.500'}>@{username}</Text>
        </Flex>
      </Flex>
    </Link>
  );
